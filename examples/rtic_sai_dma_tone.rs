...
mod app {
    ...
    struct Local {
        ...
        sai_tx: SaiTx,
        phase: u32,
        dma_buf: &'static mut [u32; DMA_BUF_LEN],
    }
    ...
    #[init(local = [buf: [u32; DMA_BUF_LEN] = [0; DMA_BUF_LEN]])]
    fn init(cx: init::Context) -> (Shared, Local) {
        ...
        let dma_buf = cx.local.buf;
        let phase = fill_tone_buffer(0, dma_buf);
        ...
        unsafe {
            channel::set_source_linear_buffer(&mut dma_chan, dma_buf);
    ...
    #[task(binds = DMA0_DMA16, local = [led, dma_chan, sai_tx, phase, dma_buf, toggle: u32 = 0], priority = 2)]
    fn dma_complete(cx: dma_complete::Context) {
        ...
        *toggle += 1;
        if toggle.is_multiple_of(172) {
            led.toggle();
        }

        // Fill buffer with next block of tone samples
        *phase = fill_tone_buffer(*phase, dma_buf);
    ...
    fn fill_tone_buffer(mut phase: u32, buf: &mut [u32; DMA_BUF_LEN]) -> u32 {
